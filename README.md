...
> Yunzai Bot 插件 —— 米游社实时体力查询 + 星铁全部深渊三合一渲染 + 独立深渊查询，支持原神 / 星穹铁道 / 绝区零 / 崩坏3 四游戏。
...
### 星铁全部深渊（三合一）
- 支持 `*全部深渊` / `深渊总览` / `深渊汇总` / `星铁深渊` 等指令
- 一次性查询星铁三个深渊模式：混沌回忆、虚构叙事、末日幻影
- 三列并排渲染成一张图，包含角色、光锥、遗器、天赋等详细信息
- 支持上期数据查询：`*全部深渊 上期`
- 别问异相为什么没加，因为不够格子！
...
- 角色头像、光锥、遗器均使用插件内 `resources/meta-sr/` 资源
