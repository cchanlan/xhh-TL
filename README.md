...
> Yunzai Bot 插件 —— 米游社实时体力查询（原神 / 星穹铁道 / 绝区零）；星铁全部深渊四合一；原神全部深渊三合一；幻想真境剧诗。
...
### 体力查询
- 支持 `#体力` / `#tl` / `#体力总览` 同时查询原神 / 星铁 / 绝区零体力
- 各游戏别名：`#原神体力` / `#ystl` · `#星铁体力` / `#xttl` · `#绝区零体力` / `#zzztl`
- 支持 @他人查询体力
- 支持 `show_all_bindings` 多账号模式：同一 QQ 绑定的多个 UID 渲染进同一张图，多游戏合并转发
- 支持 `#开启体力uid` / `#关闭体力uid` 控制查询体力时是否显示游戏UID（每个用户独立设置）
- 支持 `#开启绝区零体力` / `#关闭绝区零体力` 控制「体力总览」是否包含绝区零（每个用户独立设置，默认显示）；关闭后无论本人或他人 @ 查询，该用户总览均不显示绝区零；单独 `#绝区零体力` 不受影响
- 图形化卡片输出，包含体力恢复倒计时、委托派遣、每日实训等信息
- **立绘卡**（`tl_card_style: portrait`）：原神 / 星铁 / 绝区零均为右侧大立绘样式；绝区零角色图默认取自 `resources/zzzrole`（可配置 `tl_zzz_portrait_folder`）
