...
实测成功率 **100%**（单轮约 30%，靠多轮重试兜底），单轮约 9 秒。
...
8. **`verify()` 要反复调，且判据不能只看 canvas 存在** —— 极验 initGeetest 后到出题之间有一段
   本地人机检测（实测约 4.4 秒零网络请求），此间 canvas 元素已存在但内容空白，
   抓到的 PNG 只有约 2KB。所以是「循环 verify + 按 PNG 体积探图」，不是定时等待
9. **不要改成 sleep 定值** —— 单轮白等 11 秒，占原耗时的 70%。条件等待实测单轮 9 秒

## 耗时构成（实测）

| 阶段 | 耗时 |
|---|---|
| 申请极验 + 启浏览器 + goto | 约 1.8s |
| 等滑块组件渲染（循环 verify + 探图） | 约 1.5s |
| 算缺口（Python 冷启） | 约 0.2s |
| 拖动 | 约 2.8s |
| 等结果（轮询） | 约 1.7s |
| **单轮合计** | **约 9s** |

缺口识别的两种算法实测耗时几乎相同（差分法 4.6ms vs ddddocr 5.9ms），
**换识别库对总耗时没有意义** —— 瓶颈全在浏览器与人机检测等待上。
