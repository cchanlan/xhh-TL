...
async function loadMiaoModules() {
  ...
  try {
    ...
    console.error('[xhh-TL][miniStory] 加载 miao-plugin 模块失败:', err);
    return false;
  }
}
...
// 小混沌主函数
export async function miniStory(e) {
  try {
    // 加载 miao-plugin 模块
    ...
    // 兼容原版 miao-plugin
    if (!mys.getChallengeStory) {
      mys.getChallengeStory = async function(type = 1) {
    ...
    let chaosRes;
    try {
      chaosRes = await mys.getChallengeStory(type);
    } catch (err) {
      logger.error('[xhh-TL][miniStory] 获取虚构叙事数据失败:', err);
    ...
    if (!chaosData) {
      e.reply(`暂未获得${type === 2 ? '上期' : '本期'}虚构叙事挑战数据...`);
      return false;
    }
    ...
    try {
      ...
      logger.debug('[xhh-TL][miniStory] 刷新角色信息失败:', err.message);
    }
    ...
    console.error('[xhh-TL][miniStory] error:', err);
