...
async function loadMiaoModules() {
  ...
  try {
    ...
    console.error('[xhh-TL][miniBoss] 加载 miao-plugin 模块失败:', err);
    return false;
  }
}
...
// 小混沌主函数
export async function miniBoss(e) {
  try {
    // 加载 miao-plugin 模块
    ...
    // 兼容原版 miao-plugin
    if (!mys.getChallengeBoss) {
      mys.getChallengeBoss = async function(type = 1) {
    ...
    let chaosRes;
    try {
      chaosRes = await mys.getChallengeBoss(type);
    } catch (err) {
      logger.error('[xhh-TL][miniBoss] 获取末日幻影数据失败:', err);
      e.reply('获取末日幻影数据失败，请稍后重试');
      return false;
    }
    ...
    if (!chaosData) {
      e.reply(`暂未获得${type === 2 ? '上期' : '本期'}末日幻影挑战数据...`);
      return false;
    }
    ...
    try {
      ...
      logger.debug('[xhh-TL][miniBoss] 刷新角色信息失败:', err.message);
    }
    ...
    const renderData = {
      ...
      floorScore: firstFloor.score || 0,
      node1Score: firstFloor.node1?.score || 0,
      node2Score: firstFloor.node2?.score || 0,
      node3Score: firstFloor.node3?.score || 0,
    ...
    try {
      ...
      logger.error('[xhh-TL][miniBoss] 渲染末日幻影失败:', err);
      e.reply('末日幻影数据渲染失败，请稍后重试');
      return false;
    }

    return true;
  } catch (err) {
    console.error('[xhh-TL][miniBoss] error:', err);
